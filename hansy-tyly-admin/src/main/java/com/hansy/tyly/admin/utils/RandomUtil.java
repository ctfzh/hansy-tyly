...
public final class RandomUtil {
    ...
    public static String prodNo(){
        ...
        Long date = new Date().getTime();
        String str = Long.toHexString(date);
        String result = "PROD"+str+s.substring(17, 18) + s.substring(19, 23) + s.substring(24);
        return result.toUpperCase();
    }

    public static String numberKey(int length){
        String date = System.currentTimeMillis() + "";
        int dateStart = date.length() - (length - 4);
        return date.substring(dateStart, date.length() - 1) + number(4);
    }
